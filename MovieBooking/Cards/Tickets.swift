...
//  MovieBooking
//
//  Created by Guilherme on 07/08/23.
//
...
struct InfiniteStackView: View {
    ...
    var body: some View {
        ...
        .zIndex( getIndex() == 0 && offset > 100 ? Double(CGFloat(tickets.count) - getIndex())
                 - 1 : Double(CGFloat(tickets.count) - getIndex()))
    ...
    func removeAndAdd() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1 ) {
            var upadatedTicket = ticket
            upadatedTicket.id = UUID().uuidString
            
            tickets.append(upadatedTicket)
